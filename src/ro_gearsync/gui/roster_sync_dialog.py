...
class RosterSyncDialog(ctk.CTkToplevel):
    ...
    def _build_summary(self) -> None:
        ...
        ctk.CTkLabel(
            ...
            text=(
                ...
                f"共 {len(self._plan.changes)} 筆變更待確認"
                "（未勾選／未選擇的項目不會寫入）"
            ),
    ...
    def _build_buttons(self) -> None:
        ...
        bar.grid(row=2, column=0, padx=12, pady=(6, 12), sticky="ew")
        ctk.CTkButton(
    ...
    def _apply(self) -> None:
        ...
        if not messagebox.askyesno(
            "確認寫入",
            f"將套用 {chosen} 筆變更到 guild_scores 與 league_scores，"
            "寫入前會自動備份兩份檔案。\n\n確定要寫入嗎？",
            parent=self,
        ):
            return
        ...
        try:
            report = apply_plan(self._plan, decisions)
